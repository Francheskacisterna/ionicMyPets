...
export class ProductService {
  ...
  // Obtener todos los productos (de la API o SQLite según disponibilidad)
  async getProducts(): Promise<void> {
    const apiAvailable = await this.isApiAvailable();

    if (apiAvailable) {
      // Obtén los productos de la API
      this.getProductsAPI().subscribe(
        (products) => {
          this.productsSubject.next(products); // Emite los productos a través de `products$`
        },
        (error) => {
          console.error('Error al obtener productos desde la API:', error);
        }
      ...
      // Obtén los productos desde SQLite
      const products = await this.getProductsSQLite();
      this.productsSubject.next(products); // Emite los productos a través de `products$`
    }
  }
